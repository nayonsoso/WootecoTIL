...
| 날짜 | 내용 | 완료 | 중요 |
|--------|--------|--------|--------|
| 📆24.03.05 TUE | Stack보다는 Deque를 써야하는 이유 | |  |
| 📆24.03.06 WED | VO란 | |  |
|  | VO와 Dto의 차이 | | |
| 📆24.03.07 THU | 가변인자란 | |  |
| 📆24.03.08 FRI | stream.anyMatcher의 사용법 | |  |
| 📆24.03.11 MON | 객체 안에서 캐싱을 하는 방법 | |  |
| 📆24.03.12 TUE | 상태 패턴 | |  |
|  | 전략 패턴 | | |
| 📆24.03.13 WED | reduce와 mapToInt.sum() 간의 차이 | |  |
| | flatmap의 사용법 | | |
| 📆24.03.14 THU | 팩토리 패턴 | |  |
|  | 팩토리 클래스 | | |
| 📆24.03.15 FRI | 가변 객체와 불변 객체 | | |
|  | 쓰레드 안정성과 불변 객체의 연관성 | | |
| 📆24.03.18 MON | EOF 컨벤션을 지켜야 하는 이유 | |  |
...
| 📆24.03.20 WED | autocloseable과 try-with-resource | |  |  
| 📆24.03.21 THU | List와 Set의 contains 성능 비교 | |  |
| 📆24.03.22 FRI | 표준 예외의 종류 | |  |
| 📆24.03.25 MON | CheckedException과 UncheckedException의 차이 | |  |
| 📆24.03.26 TUE | 제네릭과 와일드 카드의 차이 | |  |
|  | 제네릭 클래스나 함수에서 적는 `<T> T`의 의미 | | |
| 📆24.03.27 WED | 무분별한 static 사용을 지양해야 하는 이유 | |  |
| 📆24.03.28 THU | JUnit과 AssertJ의 차이 | |  |  
| 📆24.03.29 FRI | 테스트 코드에서 검증하려는 함수 외의 함수를 호출한다면, 이는 의존적인 테스트인가? | |  |
| 📆24.04.01 MON | 테스트 더블의 종류 | |  |
